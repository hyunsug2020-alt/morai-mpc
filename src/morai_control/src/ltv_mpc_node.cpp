...
public:
    LTVMPCNode() {
        ...
        ego_sub_    = nh.subscribe("/localization/ego_status", 1, &LTVMPCNode::egoCallback, this);
        ...
        ROS_INFO("LTV-MPC: GPS/IMU ESKF localization, global search on first step");
    }
...
private:
    ...
    void loadPath(const std::string& file) {
        ...
        ROS_INFO("Loaded %d waypoints", n);
    }

    // ── ESKF localization 콜백 ──────────────────────────────
    ...
    void controlLoop(const ros::TimerEvent&) {
        ...
        // 2. 배치 행렬
        double v_ref = std::max(0.5, cur_v_);
        ...
        for (int i = 0; i < cfg_.N; ++i) {
            int ki  = std::min(nearest_idx + i + 1, (int)waypoints_k_.size() - 1);
            int ki0 = std::min(nearest_idx + i,     (int)waypoints_k_.size() - 1);
        ...
        // 4. kappa: 참조값 기반 보정 (적분 누적 없음)
        current_kappa_ = kappa_ref + solution[0] * cfg_.Ts;
    ...
    bool   search_initialized_ = false;  // 첫 스텝 전체 탐색 플래그
    bool   is_odom_received_  = false;

    // 후처리 파라미터
    double curve_spd_hdg_thresh_ = 0.20;
    double curve_spd_gain_       = 0.08;
    double curve_spd_min_ratio_  = 0.60;
    double overshoot_dist_       = 0.10;
    double overshoot_damp_       = 0.30;
    double osc_cte_db_           = 0.05;
    double osc_hdg_db_           = 0.08;
    double osc_damp_             = 0.40;
    double near_cte_thresh_      = 0.08;
    double near_hdg_thresh_      = 0.08;
    double near_steer_damp_      = 0.85;
    double near_v_scale_         = 0.97;
    double max_steer_rate_       = 40.0;
