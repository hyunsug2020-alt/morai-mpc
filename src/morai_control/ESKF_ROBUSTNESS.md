...
- LIO-SAM is aligned to the MORAI map while GPS is healthy and supplies
  ...
  second.
- The validated wheel/IMU odometry supplies signed forward speed continuously
  and an anchored relative-position correction during GPS shadow only.
- In MORAI Real Time Mode, ESKF consumes the pure-odometry diagnostic
  `motion_rate_scale` so IMU propagation, GPS velocity, delay compensation,
  and LIO/odometry synchronization use vehicle-physics time rather than the
  faster source timestamp clock.
- All ROS sensor callbacks are serialized around the shared state and
  covariance, preventing concurrent IMU/GPS/odometry updates.

Parameters are in `../eskf/config/eskf_robust.yaml`. The noise test is:
...
The production ROS wrapper can also be replayed against recorded MORAI motion:

```bash
rosrun morai_control validate_eskf_rosbag_faults.py --seed 20260803
```

The 312.19-second regression uses six recorded moving bags, continuous added
GPS/IMU noise, 8-30 second random GPS shadows, 18-35 m GPS jumps, IMU spikes,
and the recorded pure odometry. Four deterministic random seeds passed; their
overall position RMSE was 0.770-1.301 m, GPS-shadow p95 was 1.661-3.840 m,
maximum GPS-shadow error was 1.816-4.483 m, and yaw RMSE was 0.405-0.547 deg.
