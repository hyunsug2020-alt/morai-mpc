...
class LioOdometryExperiment:
    ...
    def __init__(self):
        ...
        self.sync_mode = rospy.get_param(
            "~synchronization_mode", "arrival_monotonic")
        if self.sync_mode not in ("arrival_monotonic", "source_stamp"):
            raise ValueError(
                "~synchronization_mode must be arrival_monotonic or source_stamp")
        ...
        self.yaw_warning_deg = float(rospy.get_param(
            "~yaw_warning_deg", 2.0))
        self.lidar_x = float(rospy.get_param("~lidar_x", 1.045))
        self.lidar_y = float(rospy.get_param("~lidar_y", 0.000))
        self.source_pose_frame = rospy.get_param(
            "~source_pose_frame", "lidar")
        self.monitor_gps = bool(rospy.get_param("~monitor_gps", True))
        self.monitor_degeneracy = bool(rospy.get_param(
            "~monitor_degeneracy", True))
        self.estimate_label = rospy.get_param(
            "~estimate_label", "LIO-SAM")
    ...
    def truth_callback(self, msg):
        ...
        sample = {
            "source_stamp": message_stamp(msg.header),
    ...
    def gps_callback(self, msg):
        with self.lock:
            self.last_gps_receipt = time.monotonic()
            self.gps_status = int(msg.status)

    def incremental_callback(self, msg):
        with self.lock:
            self.last_incremental_receipt = time.monotonic()
            self.degenerate = bool(msg.pose.covariance[0] >= 0.5)

    def _nearest_truth(self, synchronization_time):
        if not self.truth_queue:
            return None, math.inf
        key = (
            "source_stamp" if self.sync_mode == "source_stamp"
            else "arrival_time")
        truth = min(
            self.truth_queue,
            key=lambda sample: abs(sample[key] - synchronization_time))
        return truth, abs(truth[key] - synchronization_time)
    ...
    def _base_pose(self, msg):
        ...
        if self.source_pose_frame == "lidar":
            base_x = raw_x - (
    ...
    def odom_callback(self, msg):
        ...
        with self.lock:
            ...
            synchronization_time = (
                odom_stamp if self.sync_mode == "source_stamp" else now)
            truth, sync_delta = self._nearest_truth(synchronization_time)
            ...
            degeneracy_fresh = (
                self.monitor_degeneracy
                and self.last_incremental_receipt is not None
            ...
            row = {
                "ros_time": "{:.9f}".format(rospy.Time.now().to_sec()),
                "elapsed_s": "{:.3f}".format(elapsed),
                "truth_stamp": "{:.9f}".format(truth["source_stamp"]),
    ...
    def _initialize_gui(self):
        ...
        try:
            ...
            self.figure.suptitle(
                "{} vs MORAI Ground Truth".format(self.estimate_label),
            ...
            self.lio_line, = self.ax_trajectory.plot(
                [], [], "b--", linewidth=1.2,
                label="{} aligned".format(self.estimate_label))
    ...
    def update_gui(self):
        ...
        if metrics is None:
            odom_age = (
                ...
                else max(0.0, time.monotonic() - last_odom_receipt))
            info = (
                "ALIGNMENT: {}\n"
                "samples: {}/{}\n"
                "odom age: {}\n"
            ...
            if not self.monitor_gps:
                gps_age_text = "disabled (validation only)"
            else:
                gps_age_text = (
    ...
    def shutdown(self):
        with self.lock:
            ...
            result = {
                ...
                "synchronization": self.sync_mode,
